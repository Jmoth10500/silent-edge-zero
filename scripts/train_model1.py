#!/usr/bin/env python3
"""
Phase 6 — train and walk-forward validate Model 1 (statistical/logistic
baseline, src/models/model1_logistic_baseline.py) against the REAL Kaggle
historical results now in the database, and score it against Model 0 (the
real de-vigged market baseline, src/models/model0_market_baseline.py) using
the real starting prices loaded alongside those results.

This is the first time either model has ever seen a real (pre-race features,
actual winner) pair. Everything upstream of this script — the fitting maths,
the walk-forward splitter, the Brier/log-loss scorer — was already tested
against synthetic fixtures; this script is what turns "the maths works" into
"here is what it actually does on real racing", honestly, including if the
answer is unflattering. See docs/RESEARCH_LAB.md RL-006 and
docs/BUILD_LOG.md for how to interpret the result.

No random splits: every train/test boundary here comes from
walk_forward_splits() (src/validation/walk_forward.py), which is purely
chronological.

Usage: python3 scripts/train_model1.py [start_date] [min_train_days] [test_window_days] [iterations]
...
def main():
    ...
    # A race is usable at all only if it has a real winner and >=3 runners
    # (softmax over 1-2 runners isn't a meaningful test of anything).
    usable = [r for r in all_races if r.winner_horse_id is not None and len(r.runners) >= 3]
    print(f"Usable for Model 1 (winner known, field >= 3): {len(usable):,} races.")

    # Model 0 additionally needs every runner in the race to have a real
    # starting price — a partial field can't be de-vigged honestly.
    ...
    header = f"{'train_end':<12}{'test_end':<12}{'n_races':>8}{'m1_brier':>10}{'m1_logloss':>12}{'m0_brier':>10}{'m0_logloss':>12}"
    print(header)
    print("-" * len(header))
    ...
    for split in splits:
        ...
        print(f"{str(split.train_end):<12}{str(split.test_end):<12}{len(test_races):>8}"
              f"{m1_b:>10.4f}{m1_l:>12.4f}{m0_b:>10.4f}{m0_l:>12.4f}")
    ...
    if all_m1_probs and all_m0_probs:
        print("\nModel 1 vs Model 0 (the actual research question — Section 57):")
        if brier_score(all_m1_probs, all_m1_outcomes) < brier_score(all_m0_probs, all_m0_outcomes):
            print("  Model 1 beat the market baseline on Brier score, out-of-sample, walk-forward.")
        else:
            print("  Model 1 did NOT beat the market baseline on Brier score, out-of-sample. "
                  "The market is hard to beat — this is a normal, informative result, not a failure "
                  "of the pipeline.")

    print("\nCalibration (Model 1, pooled, 10 bins):")
    for b in calibration_curve(all_m1_probs, all_m1_outcomes, n_bins=10):
